                    ...
                    <div class="tabs -active-purple-2 js-tabs">
                        ...
                        <div class="tabs__content py-30 px-30 js-tabs-content">
                            ...
                            <div class="tabs__pane -tab-item-2 is-active">
                                ...
                                <div class="row y-gap-20 justify-between pt-5">
                                    <div class="col-auto sm:w-1/1">
                                        <h1 class="text-30 lh-12 fw-700">Item Events</h1>
                                        <div class="mt-10"><i class="icon icon-save-money"></i> Budget: <strong>
                                                {{ number_format($event->items->sum('amount')) }} TZS</strong><span
                                                class="text-success"> Paid: <strong>
                                                    {{ number_format($event->items->sum('paid')) }} TZS</strong></span>
                                        </div>
                                    </div>

                                    <div class="col-auto sm:w-1/1">
                                        <button data-toggle="modal" data-target="#add_item"
                                            class="button -pulse -sm -black text-blue-1 sm:w-1/1">
                                            <i class="icon icon-email mr-5"></i> Add Item
                                        </button>
                                    </div>
                                </div>
                                    ...
                                    <table class="table table-striped table-sm data-table">
                                        ...
                                        <thead class="thead" style="background-color: #d3ccbc;">
                                            <tr>
                                                ...
                                                <th class="text-center" style="color:white">Paid</th>
                                                <th class="text-center" style="color:white">Balance</th>
                                                <th style="color:white">Status</th>
                                                <th class="text-center" style="color:white">{!! trans('usersmanagement.users-table.actions') !!}</th>
                                            </tr>
                                        </thead>
                                        <tbody id="users_table">
                                            @foreach ($event->items as $item)
                                                <tr>
                                                    ...
                                                    <td class="p-2">
                                                        <p class="p-0">{{ $item->name }}</p>
                                                        <p class="p-0" style="font-size: 9px; color: orange">
                                                            {{ $item->item_type->name }}</p>
                                                    </td>
                                                    <td class="p-2 text-right">{{ number_format($item->amount) }}</td>
                                                    <td class="p-2 text-right">{{ number_format($item->paid) }}</td>
                                                    <td class="p-2 text-right">
                                                        {{ number_format($item->amount - $item->paid) }}</td>
                                                    <td class="p-2">
                                                        <span
                                                            class="badge badge-{{ $item->amount > $item->paid ? 'warning' : 'success' }}">{{ $item->amount > $item->paid ? 'Pending' : 'Paid' }}</span>
                                                    </td>
                                                    <td class="p-2">
                                                        <div class="shopCart-footer__item">
                                                            <button class="button -sm -purple-3 text-purple-1 mr-5"
                                                                data-toggle="modal"
                                                                data-target="#item_edit_{{ $item->id }}">Edit</button>
                                                            <button class="button -sm -purple-3 text-purple-1 mr-5"
                                                                data-toggle="modal"
                                                                data-target="#item_pay_{{ $item->id }}">Payment</button>
                                                            <button class="button -sm -outline-red-1 text-red-1"
                                                                data-toggle="modal"
                                                                data-target="#delete_item_{{ $item->id }}">Delete</button>
                                                        </div>
                                                    </td>
                                                </tr>
                                            @endforeach
