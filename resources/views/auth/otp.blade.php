...
<link href="https://fonts.googleapis.com/css2?family=Pacifico&display=swap" rel="stylesheet">
        ...
        <section class="form-page js-mouse-move-container">
            <div class="form-page__img" style="background-image: url('{{ asset('img/home-9/hero/bg.png') }}');background-size: cover; background-position: center; background-repeat: no-repeat;">
              ...
              <div class="row justify-center items-center">
                <div class="col-xl-6 col-lg-8">
                  ...
                  <div class="px-50 py-50 md:px-25 md:py-25 bg-white shadow-1 rounded-16">
                    ...
                    <h3 style="display: flex; justify-content: center; font-weight:normal" class="text-30 lh-13">{{ __('Verify OTP')}}</h3>
                    {{-- <p class="mt-10">Tumetuma OTP ya tarakimu 6 kwenda {{$phone}} <br> <a href="/login" class="text-purple-1">{{ __('Change Number')}}</a></p> --}}

                    <form class="contact-form respondForm__form row y-gap-20 pt-30" method="POST" action="{{ route('validate_otp') }}">
                        @csrf
                      <div class="col-12">
                        {{-- <label class="text-16 lh-1 fw-500 text-dark-1 mb-10">OTP   @if ($errors->has('otp'))<span style="color: red">{{ $errors->first('otp') }}</span>@endif</label> --}}
                        <input type="text" minlength="6" style="text-align: center;" required maxlength="6" name="otp" placeholder="* * * * * *"  onkeypress='validate(event)'>
                      </div>
                      <div class="col-12">
                        <button type="submit" name="submit" id="submit" class="button -md -black text-dark-1 fw-500 w-1/1" style="background-color: #003366;">
                          {{ __('Verify')}}
                        </button>
                      </div>
                    </form>
